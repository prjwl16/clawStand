...
// ------------------------------------------------------------------
// PLANNER — I4 upgrade: L4 task decomposition.
// Runs BEFORE the specialists, decides which to call and what each
// should focus on. Everything after the planner is a plain execution
// of that plan.
// ------------------------------------------------------------------
...
function defaultPlan(url, repoUrl) {
  const agents = [{ name: 'productInspector', focusInstructions: '' }];
  const skipReasons = [];
  if (repoUrl) {
    agents.push({ name: 'repoAuditor', focusInstructions: '' });
  } else {
    skipReasons.push({ agent: 'repoAuditor', reason: 'no repo provided' });
  }
  return skipReasons.length ? { agents, skipReasons } : { agents };
}
...
AVAILABLE SPECIALISTS
  productInspector — analyzes the rendered live product HTML. ALWAYS include.
  repoAuditor      — queries the GitHub REST API (package.json, requirements.txt, pyproject.toml, go.mod, README, last 10 commits). Include ONLY IF a repo URL is provided.

WRITE FOCUS INSTRUCTIONS (1–2 sentences per agent) based on signals you can see:
  ...
  - Repo name hints at JS/TS or unclear → emphasize package.json for agent framework signals (langgraph, crewai, @ai-sdk, ai).
  ...
  - Live URL is a marketing domain (plain .com, .ai, .io with no app subdomain) → tell productInspector to distinguish *described* product from *actual accessible* agent output; marketing copy alone should not score above L2 on real output shipping.
...
{
  "agents": [
    { "name": "productInspector", "focusInstructions": "..." }${repoUrl ? `,
    { "name": "repoAuditor", "focusInstructions": "..." }` : ''}
  ]${repoUrl ? '' : `,
  "skipReasons": [
    { "agent": "repoAuditor", "reason": "no repo URL was provided" }
  ]`}
}`;
  ...
  try {
    ...
    const raw = JSON.parse(stripJson(msg.content[0].text));
    return PlanSchema.parse(raw);
  } catch (e) {
    console.log('[planner] fell back to default plan:', e && e.message);
...
async function productInspector(html, url, focusInstructions) {
  ...
  const plannerBlock = focusInstructions && focusInstructions.trim()
    ? `PLANNER FOCUS NOTE (additional context for this submission, from the upstream planner agent — consider it when scoring but do NOT let it override the rubric):
${focusInstructions.trim()}

`
    : '';

  const prompt = `${plannerBlock}You are the PRODUCT INSPECTOR judging a hackathon submission against the GrowthX MaaS rubric. MaaS = Multi-Agent-as-a-Service. This rubric exists to evaluate agent systems, NOT general-purpose SaaS. A polished non-agent product is a BAD submission under this rubric, no matter how beautiful it is.
...
module.exports = { RUBRIC, planExecution, productInspector, repoAuditor, pitchWriter };
